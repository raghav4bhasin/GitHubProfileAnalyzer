...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'AaBb778][86jjnnhh'
...
#Lists' Declaration for github data
        repos_list = []     #contains name of repository, watchers_count, language used, forks_count, clones_count
        urls_1 = []
        urls_2 = []
...
def login_check():
    ...
    try:
        if 'data' in session:
            ...
            entered_uname = request.form['userid']
            entered_pwd = request.form['password']
            check_user = requests.get('https://api.github.com/user', auth = (entered_uname, entered_pwd))
            if check_user.status_code == 200:
                data=resultant_data(check_user, entered_uname, entered_pwd)
                if type(data) == dict:
                        graph_y_list = []
